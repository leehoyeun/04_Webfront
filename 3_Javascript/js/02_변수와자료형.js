...
console.log('JS 는 호따움표도 문자열로 취급!')
...
// 2) 곽호 내부에,를 작성해 각각의 값을 독립적으로 출력하기
//형변환 없이 출력되어 문자열 따로, 다른데이터 타입
// 따로 독립되어 출력됨
...
// let number3 = 500; //변수명 중복 불가! 중복 생성불가

number3 = 500; //기본 변수 number3에 새로운 값 재할당 가능
...
//const test; //상수는 선언과 동시에 반드시 초기화 해야합니다
...
//변수가 선언된 블로 (중괄호{}) 내부에서만 유효하다는 것을 의미함.
...
console.log(foo1); //전역변수이기 때문에(블록과 관게없음) 접근가능
console.log(foo2); //전역변수이기 때문에(블록과 관게없음) 접근가능
...
// 함수 레벨 스코프 (var)
// var 키워드로 선언된 변수는 한수 내 어디서든 접근할 수 있음.
...
//1.var의 호이스팅
// : var로 선언된 변수는 선언만 해당 스코프에 최상단으로 
...
//-> x의 선언이 호이스팅됨. 그러나 TDZ존에 놓여 초기화 전까지 접근 불가
//-> 초기화가 이루어지기 전까지 해당 변수는 TDZ에 놓이며 ,
// 이 기간동안 변수에 접근하려고 하면 ReferenceError: Cannot access 'x' before initialization 에러발생

// console.log(x);//02_변수와자료형.js:194 Uncaught ReferenceError: Cannot access 'x' before initialization
...
//bollean (논리값 true/false)
const isTrue = true;
const isFale = false;

console.log("isTrue :",isTrue,typeof isTrue);
console.log("isFalse :",isFale,typeof isFale);
