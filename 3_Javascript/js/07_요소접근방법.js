...
divs[0].style.backgroundColor= "rgb(135, 221, 101)";
...
function nameTest(){
  ...
  document.getElementById("name-div").innerHTML
    ...
    //요소.innerHTML 
    //HTML 태그를 포함하여 문자열 등을 식제 HTML 요소로 해석함

    //요소.innerText
    //TEXT 내용만 요소 콘텐트 내부에 출력.
}
...
function cssTest(){
  ...
  div2 = document.querySelector("[target-div='css-div'] > div:last-child");
...
function readValue(){
  ...
  //채팅이 출력되는 배경 요소
  const bg = document.querySelector("#chattind-bg");
  ...
  if(input.value.trim().length == 0){
    ...
    input.focus(); // 해당 input창에 커서 활성화(포커싱)

    return; // 아래 수행X ,현재 수행중인 함수 종료
  }

  bg.innerHTML += `<p><span>${input.value}</span></p>`;
  bg.scrollTop = bg.scrollHeight;
  //bg.scrollTop : 현재 스크롤 위치
  //(스크롤이 현재 얼마만큼 내여와있는지를 나타냄)

  //bg.scrollTop = bg.scrollHeight : bg의 스크롤 전체 높이
